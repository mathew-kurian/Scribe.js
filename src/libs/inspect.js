...
function formatValue(ctx, value, recurseTimes) {
  ...
  if (keys.length === 0) {
    ...
    if (isWeakSet(value)) {
      return ctx.stylize('[WeakSet] ', 'weakset') + inspect(JSON.parse(JSON.stringify([...value])), ctx);
    }
    if (isWeakMap(value)) {
      return ctx.stylize('[WeakMap] ', 'weakmap') + inspect(JSON.parse(JSON.stringify([...value])), ctx);
    }
    if (isMap(value)) {
      return ctx.stylize('[Map] ', 'map') + inspect(JSON.parse(JSON.stringify([...value])), ctx);
    }
    if (isSet(value)) {
      return ctx.stylize('[Set] ', 'set') + inspect(JSON.parse(JSON.stringify([...value])), ctx);
    }
    if (isFunction(value)) {
